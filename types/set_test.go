...
func TestNewSet(t *testing.T) {
	assert := assert.New(t)
	s1 := NewSet()
	assert.IsType(setType, s1.Type())
	assert.Equal(uint64(0), s1.Len())

	s2 := NewSet(Number(0))
	assert.IsType(setType, s2.Type())

	s3 := NewTypedSet(MakeSetType(NumberType))
	assert.IsType(MakeSetType(NumberType), s3.Type())

	s4 := s1.Remove(Number(1))
	assert.IsType(s1.Type(), s4.Type())
}
