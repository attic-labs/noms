...
func newSetMutator(vrw ValueReadWriter) *setMutator {
	return &setMutator{newOpCache(vrw), vrw}
}
...
func (mx *setMutator) Finish() Set {
	...
	seq := newEmptySequenceChunker(mx.vrw, makeSetLeafChunkFn(mx.vrw), newOrderedMetaSequenceChunkFn(SetKind, mx.vrw), hashValueBytes)
	...
	return newSet(seq.Done(mx.vrw).(orderedSequence))
}
