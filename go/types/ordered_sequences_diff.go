...
func orderedSequenceDiffLeftRight(last orderedSequence, current orderedSequence, changes chan<- ValueChanged, stopChan <-chan struct{}) bool {
	...
	for lastCur.valid() && currentCur.valid() {
		...
		for lastCur.valid() && currentCur.valid() &&
			...
			if currentKey.Less(lastKey) {
				if !sendChange(changes, stopChan, ValueChanged{DiffChangeAdded, currentKey.v, nil, getCurrentValue(currentCur)}) {
					return false
				}
				currentCur.advance()
			} else if lastKey.Less(currentKey) {
				if !sendChange(changes, stopChan, ValueChanged{DiffChangeRemoved, lastKey.v, getCurrentValue(lastCur), nil}) {
					return false
				}
				lastCur.advance()
			} else {
				if !sendChange(changes, stopChan, ValueChanged{DiffChangeModified, lastKey.v, getCurrentValue(lastCur), getCurrentValue(currentCur)}) {
					return false
				}
				lastCur.advance()
				currentCur.advance()
	...
	for lastCur.valid() {
		if !sendChange(changes, stopChan, ValueChanged{DiffChangeRemoved, getCurrentKey(lastCur).v, getCurrentValue(lastCur), nil}) {
			return false
		}
		lastCur.advance()
	}
	for currentCur.valid() {
		if !sendChange(changes, stopChan, ValueChanged{DiffChangeAdded, getCurrentKey(currentCur).v, nil, getCurrentValue(currentCur)}) {
			return false
		}
		currentCur.advance()
	}
