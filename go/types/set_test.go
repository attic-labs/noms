...
func TestNewSet(t *testing.T) {
	...
	s2 := s.Edit().Remove(Number(1)).Build(nil)
...
func TestSetLen(t *testing.T) {
	...
	s2 := s1.Edit().Insert(Bool(false)).Build(nil)
	...
	s3 := s2.Edit().Remove(Bool(true)).Build(nil)
...
func TestSetEmptyInsert(t *testing.T) {
	...
	s := NewSet()
	assert.True(s.Empty())
	s = s.Edit().Insert(Bool(false)).Build(nil)
...
func TestSetEmptyInsertRemove(t *testing.T) {
	...
	assert.False(s.Empty())
	assert.Equal(uint64(1), s.Len())
	s = s.Edit().Remove(Bool(false)).Build(nil)
	assert.True(s.Empty())
	assert.Equal(uint64(0), s.Len())
}
...
func TestSetInsert(t *testing.T) {
	...
	s = s.Edit().Insert(v1).Build(nil)
	assert.True(s.Has(v1))
	s = s.Edit().Insert(v2).Build(nil)
	assert.True(s.Has(v1))
	assert.True(s.Has(v2))
	s2 := s.Edit().Insert(v3).Build(nil)
	assert.True(s.Has(v1))
	assert.True(s.Has(v2))
...
func TestSetInsert2(t *testing.T) {
	...
	doTest := func(incr, offset int, ts testSet) {
		...
		run := func(from, to int) {
			actual := ts.Remove(from, to).toSet().Edit().Insert(ts[from:to]...).Build(nil)
...
func TestSetInsertExistingValue(t *testing.T) {
	...
	original := ts.toSet()
	actual := original.Edit().Insert(ts[0]).Build(nil)
...
func TestSetRemove(t *testing.T) {
	...
	s = s.Edit().Remove(v1).Build(nil)
	assert.False(s.Has(v1))
	assert.True(s.Has(v2))
	assert.True(s.Has(v3))
	s2 := s.Edit().Remove(v2).Build(nil)
	assert.False(s.Has(v1))
	assert.True(s.Has(v2))
	assert.True(s.Has(v3))
...
func TestSetRemove2(t *testing.T) {
	...
	doTest := func(incr, offset int, ts testSet) {
		...
		run := func(from, to int) {
			...
			actual := whole.Edit().Remove(ts[from:to]...).Build(nil)
			assert.Equal(expected.Len(), actual.Len())
			assert.True(expected.Equals(actual))
			diffSetTest(assert, expected, actual, 0, 0)
		}
...
func TestSetRemoveNonexistentValue(t *testing.T) {
	...
	ts := getTestNativeOrderSet(2)
	original := ts.toSet()
	actual := original.Edit().Remove(Number(-1)).Build(nil) // rand.Int63 returns non-negative values.
...
func TestSetFirst(t *testing.T) {
	...
	s := NewSet()
	assert.Nil(s.First())
	s = s.Edit().Insert(Number(1)).Build(nil)
	assert.NotNil(s.First())
	s = s.Edit().Insert(Number(2)).Build(nil)
	assert.NotNil(s.First())
	s2 := s.Edit().Remove(Number(1)).Build(nil)
	assert.NotNil(s2.First())
	s2 = s2.Edit().Remove(Number(2)).Build(nil)
	assert.Nil(s2.First())
}
...
func TestSetIter(t *testing.T) {
	...
	s.Iter(func(v Value) bool {
		_, ok := v.(Number)
		assert.True(ok)
		acc = acc.Edit().Insert(v).Build(nil)
		return false
	})
...
func TestSetType(t *testing.T) {
	...
	s2 = s.Edit().Insert(Number(0), Number(1)).Build(nil)
	assert.True(TypeOf(s).Equals(TypeOf(s2)))

	s3 := s.Edit().Insert(Bool(true)).Build(nil)
	assert.True(TypeOf(s3).Equals(MakeSetType(MakeUnionType(BoolType, NumberType))))
	s4 := s.Edit().Insert(Number(3), Bool(true)).Build(nil)
...
func TestSetModifyAfterRead(t *testing.T) {
	...
	fst := set.First()
	set = set.Edit().Remove(fst).Build(nil)
	assert.False(set.Has(fst))
	assert.True(set.Has(set.First()))
	set = set.Edit().Insert(fst).Build(nil)
...
func TestSetRemoveLastWhenNotLoaded(t *testing.T) {
	...
	for len(ts) > 0 {
		last := ts[len(ts)-1]
		ts = ts[:len(ts)-1]
		ns = reload(ns.Edit().Remove(last).Build(nil))
