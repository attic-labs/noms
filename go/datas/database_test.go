...
func (suite *DatabaseSuite) TestDatabaseCommitMerge() {
	...
	ds1First := ds1
	suite.NoError(err)
	ds1, err = suite.db.CommitValue(ds1, types.NewMapEditor(v).Set(types.String("Friends"), types.Bool(true)).Build(suite.db))
	suite.NoError(err)
	...
	// Merge policies
	newV := types.NewMapEditor(v).Set(types.String("Friends"), types.Bool(false)).Build(suite.db)
	...
	newV = types.NewMapEditor(v).Set(types.String("Friends"), types.Number(47)).Build(suite.db)
