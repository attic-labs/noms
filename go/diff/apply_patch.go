...
func (stack *patchStack) updateNode(top *stackElem, parent types.Value) types.Value {
	...
	case types.IndexPath:
		...
		case types.Map:
			switch top.changeType {
			case types.DiffChangeAdded:
				return el.Edit().Set(part.Index, top.newValue).Build(nil)
			case types.DiffChangeRemoved:
				return el.Edit().Remove(part.Index).Build(nil)
			case types.DiffChangeModified:
				if part.IntoKey {
					newPart := types.IndexPath{Index: part.Index}
					ov := newPart.Resolve(parent, nil)
					return el.Edit().Remove(part.Index).Set(top.newValue, ov).Build(nil)
				}
				return el.Edit().Set(part.Index, top.newValue).Build(nil)
			}
		case types.Set:
			if top.oldValue != nil {
				el = el.Edit().Remove(top.oldValue).Build(nil)
			}
			if top.newValue != nil {
				el = el.Edit().Insert(top.newValue).Build(nil)
			}
			return el
		}
	case types.HashIndexPath:
		...
		case types.Set:
			switch top.changeType {
			case types.DiffChangeAdded:
				return el.Edit().Insert(top.newValue).Build(nil)
			case types.DiffChangeRemoved:
				return el.Edit().Remove(top.oldValue).Build(nil)
			case types.DiffChangeModified:
				return el.Edit().Remove(top.oldValue).Insert(top.newValue).Build(nil)
			}
		case types.Map:
			...
			case types.DiffChangeAdded:
				k := top.newKeyValue
				return el.Edit().Set(k, top.newValue).Build(nil)
			case types.DiffChangeRemoved:
				return el.Edit().Remove(k).Build(nil)
			case types.DiffChangeModified:
				if part.IntoKey {
					v := el.Get(k)
					return el.Edit().Remove(k).Set(top.newValue, v).Build(nil)
				}
				return el.Edit().Set(k, top.newValue).Build(nil)
			}
		}
