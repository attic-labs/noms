...
import {MetaKey, MetaTuple, newListMetaSequence} from './meta-sequence.js';
...
suite('CompoundList', () => {
  ...
  function build(): List {
    ...
    const m1 = List.fromSequence(newListMetaSequence(db, [
      new MetaTuple(r1, new MetaKey(2), 2, null),
      new MetaTuple(r2, new MetaKey(2), 2, null)]));
    const rm1 = db.writeValue(m1);
    const m2 = List.fromSequence(newListMetaSequence(db, [
      new MetaTuple(r3, new MetaKey(2), 2, null),
      new MetaTuple(r4, new MetaKey(2), 2, null)]));
    const rm2 = db.writeValue(m2);

    const l = List.fromSequence(newListMetaSequence(db, [
      new MetaTuple(rm1, new MetaKey(4), 4, null),
      new MetaTuple(rm2, new MetaKey(4), 4, null)]));
    return l;
  }
