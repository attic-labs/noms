...
suite('CompoundList', () => {
  function build(): CompoundList {
    ...
    let m1 = new CompoundList(ms, tr, [new MetaTuple(r1, 2), new MetaTuple(r2, 4)]);
    let rm1 = writeValue(m1, tr, ms);
    let m2 = new CompoundList(ms, tr, [new MetaTuple(r3, 2), new MetaTuple(r4, 4)]);
    let rm2 = writeValue(m2, tr, ms);

    let l = new CompoundList(ms, tr, [new MetaTuple(rm1, 4), new MetaTuple(rm2, 8)]);
    return l;
  }
