...
class JsonArrayReader {
  ...
  async readList(t: Type, pkg: ?Package): Promise<ListLeaf> {
    let seq = await this.readSequence(t, pkg);
    return new ListLeaf(this._cs, t, seq);
  }

  async readSet(t: Type, pkg: ?Package): Promise<SetLeaf> {
    let seq = await this.readSequence(t, pkg);
    return new SetLeaf(this._cs, t, seq);
  }

  async readMap(t: Type, pkg: ?Package): Promise<MapLeaf> {
...
setDecodeNomsValue(decodeNomsValue);
