...
func main() {
	...
	go lib.Replicate(sub, ds, ds, func(nds datas.Dataset) {
		...
		if displayingSearchResults || !textScrolledToEnd(g) {
			...
			updateMessagesAndRefresh(g, ds, nil)
		}
	})
	...
	d.PanicIfError(g.SetKeybinding(input, gocui.KeyEnter, gocui.ModNone, func(g *gocui.Gui, v *gocui.View) (err error) {
		...
		ds = nds
		updateMessages(g, msgView, ds, sIds)
...
func (dp *dataPager) Next() (string, bool) {
	...
	return fmt.Sprintf("%s: %s", m.Author, m.Body), true
}

func updateMessages(_ *gocui.Gui, v *gocui.View, ds datas.Dataset, filterIds *types.Map) error {
	v.Clear()
	v.SetOrigin(0, 0)
	_, winHeight := v.Size()
...
func updateMessagesAndRefresh(g *gocui.Gui, ds datas.Dataset, sids *types.Map) {
	g.Execute(func(_ *gocui.Gui) error {
		v, err := g.View(messages)
		d.PanicIfError(err)
		err = updateMessages(g, v, ds, sids)
		d.PanicIfError(err)
		return nil
	})
}
